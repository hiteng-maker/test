...
#define WR_VALUE _IOW('a','a',struct data *) // ioctl write command
#define RD_VALUE _IOR('a','b',struct data *) // ioctl read command
...
static long my_ioctl(struct file *file , unsigned int cmd, unsigned long arg)
{
	switch( cmd)
	{
		case WR_VALUE: 
			...
			copy_from_user(&kernel_data,(struct data*) arg, sizeof(kernel_data));
			printk(KERN_INFO "--------Data recieved from User-------- \nname : %s\nage: %d",kernel_data.name,kernel_data.age);
		...
		case RD_VALUE:
			...
			copy_to_user((struct data* ) arg ,&kernel_data, sizeof(kernel_data));
			break;
	}
	return 0;
}
