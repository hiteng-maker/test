...
#define WR_VALUE _IOW('a','a',int32_t*) // ioctl write command
#define RD_VALUE _IOR('a','b',int32_t*) // ioctl read command


// int32_t is fixed size variable

int32_t value = 0; // value will be responsible for ioctl communication.
...
static long my_ioctl(struct file *file , unsigned int cmd, unsigned long arg)
{
	switch( cmd)
	{
		case WR_VALUE:
		      ...
		      printk(KERN_INFO "value recieved from user=%d\n",value);
		      break;
